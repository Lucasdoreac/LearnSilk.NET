...
public class Camera
{
    ...
    {
        return Calculate_LookAt_Matrix(
            position: Position, 
    ...
    private Matrix4x4 Calculate_LookAt_Matrix(Vector3 position, Vector3 target, Vector3 up)
    {
        // 1. Posição = conhecida
        ...
        Matrix4x4 translation = Matrix4x4.Identity; // Matriz identidade por padrão
        translation.M41 = -position.X; // Quarta coluna, primeira linha
        translation.M42 = -position.Y;
        translation.M43 = -position.Z;

        Matrix4x4 rotation = Matrix4x4.Identity;

        rotation.M11 = xaxis.X; // Primeira coluna, primeira linha
        rotation.M21 = xaxis.Y;
        rotation.M31 = xaxis.Z;

        rotation.M12 = yaxis.X; // Primeira coluna, segunda linha
        rotation.M22 = yaxis.Y;
        rotation.M32 = yaxis.Z;

        rotation.M13 = zaxis.X; // Primeira coluna, terceira linha
        rotation.M23 = zaxis.Y;
        rotation.M33 = zaxis.Z;

        // Retorna a matriz lookAt como uma combinação de matrizes de translação e rotação
        return translation * rotation; // Lembre-se de ler da direita para a esquerda (primeiro a translação, depois a rotação)
    }
